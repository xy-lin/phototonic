...
void ImageView::saveImage()
{
	...
	try
	{
		image = Exiv2::ImageFactory::open(currentImageFullPath.toStdWString());
		image->readMetadata();
	}
...
void ImageView::saveImageAs()
{
	...
	if (!fileName.isEmpty()) {
		try {
			exifImage = Exiv2::ImageFactory::open(currentImageFullPath.toStdWString());
		...
		if (!displayPixmap.save(fileName, 0, GData::defaultSaveQuality)) {
			...
			if (!exifError) {
				try	{
					newExifImage = Exiv2::ImageFactory::open(fileName.toStdWString());
