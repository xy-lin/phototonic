...
void Phototonic::createActions()
{
	...
	fileCopyAction = new QAction(tr("Copy Files"), this);
	fileCopyAction->setObjectName("copyFile");
	connect(fileCopyAction, SIGNAL(triggered()), this, SLOT(copyFiles()));
...
void Phototonic::createMenus()
{
	...
	toolsMenu->addAction(fileCopyAction);
...
void Phototonic::copyFiles()
{
	_allFileCount = 0;
	_copiedCount = 0;

	QString rootDst = "C:/collections/";
	...
	QDir rootDir(selectedPath);

	_copyFiles(rootDir);

	QString str("total files are: %1f \n total copied is: %2f");

	str.arg(_allFileCount, _copiedCount);

	QMessageBox msgBox;
	msgBox.information(this, tr(""), str);

}

void Phototonic::_copyFiles(QDir directoryIn)
{
	...
	foreach(QFileInfo fileInfo, list)
	{
		if (fileInfo.isFile() /*&& fileInfo.size() >= 2048*/)
		{
			...
			exifImage = Exiv2::ImageFactory::open(inName.toStdWString());
			...
			if (!iptcData.empty())
			{
				...
				for (Exiv2::IptcData::iterator iptcIt = iptcData.begin(); iptcIt != end; ++iptcIt)
				{
					if (iptcIt->tagName() == "Keywords")
					{
						...
						QString dstPath(rootDst);						
						
						QDir dir(dstPath);

						if (!dir.exists( dstPath + "/" + tagName))
						{
							dir.mkdir(tagName);
						}																	
						...
						while (true)
						{
							QFile target(dstPath);

							if (target.exists())
							{
								QString croped_fileName = dstPath.split(".", QString::SkipEmptyParts).at(0);
								croped_fileName += "_new";
								croped_fileName += ".jpg";

								dstPath = croped_fileName;
							}
							else
							{
								break;
						...
						bool done = QFile::copy(pathName, dstPath);
						...
						{							
							...
							QString str = QString("%1 \n %2").arg(pathName, dstPath);

							QMessageBox msgBox;
							msgBox.information(this, tr(""), "cannot copy" + str);							
						}
						break;
					}
				}
		...
		{
			_copyFiles(QDir(fileInfo.absoluteFilePath()));
		}
	}
}